...
class TestEpub(unittest.TestCase):
    ...
    def test_TOCHTML(self):
        ...
        def check_titles():
            chapter_nodes = self.toc_element.get_element_by_id(
                'chapters').getchildren()
        ...
        if epub.lxml_module_exists:
            create_TOC()
            check_titles()
            self.test_toc.write(os.path.join(
                TEST_DIR, 'epub_output', 'toc.html'))
        else:
    ...
    def test_TOCNCX(self):
        ...
        if epub.lxml_module_exists:
            ...
            checkTitles()
            self.test_toc.write(os.path.join(
                TEST_DIR, 'epub_output', 'toc_ncx.xml'))
        else:
    ...
    def test_create_epub(self):
        ...
        epub_directory = os.path.join(
            TEST_DIR, 'epub_output', 'epub files' + epub_dir_ending)
